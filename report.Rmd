...
output:
  word_document:
    ...
    fig_height: 10
    fig_width: 10
    reference_docx: raw/exchange_style_8.docx
    toc: TRUE
params:
  report_title: ""
---
...
The overall percentage agreement based on modal ages for all readers 
is `r round(sum_stat[2],1)`%, with an overall CV of `r round(sum_stat[1],1)`%.
...
# Output table
pander(style_table1(cv_tab), missing = "-")
...
# Output table
pander(style_table1(pa_tab[,c(1:ncol(pa_tab))]), missing="-")
...
# Output table
pander(style_table1(rb_tab), missing = "-")
...
**Results by area (or stock?)** 

```{r num_strata}

# NUMEBR OF AGE READINGS PER STRATA - All readers #################################

# Table caption
set.caption('**Table X:** Number of age readings per strata. (area or stock?)')

# Output table
pander(style_table1(num_st_comp), missing = "-")

```

```{r cv_strata}

# CV PER STRATA - All readers #################################################

# Table caption
set.caption('**Table X:** CV per strata. (area or stock?)')

# Output table
pander(style_table2(cv_st_tab), missing = "-")

```

```{r pa_strata}

# PA PER STRATA - All readers #################################################

# Table caption
set.caption('**Table X:** Percentage Agreement per strata. (area or stock?)')

# Output table
pander(style_table2(pa_st_tab), missing = "-")

```

```{r rb_strata}

# RELATIVE BIAS PER STRATA - All readers ######################################

# Table caption
set.caption('**Table X:** Relative Bias per strata. (area or stock?)')

# Output table
pander(style_table2(rb_st_tab), missing = "-")
...
# NUMBER OF AGE READINGS PER STRATA - Advanced readers ############################

# Table caption
set.caption('**Table X:** Number of age readings per strata for advanced readers.')

# Output table
pander(style_table1(num_st_comp_ex), missing = "-")

```

```{r cv_strata_ex}

# CV PER STRATA - Advanced readers ############################################

# Table caption
set.caption('**Table X:** CV per strata.')

# Output table
pander(style_table2(cv_st_tab_ex), missing = "-")

```

```{r pa_strata_ex}

# PA PER STRATA - Advanced readers ############################################

# Table caption
set.caption('**Table X:** Percentage Agreement per strata.')

# Output table
pander(style_table2(pa_st_tab_ex), missing = "-")

```

```{r rb_strata_ex}

# RELATIVE BIAS PER STRATA - Advanced readers #################################

# Table caption
set.caption('**Table X:** Relative Bias per strata.')

# Output table
pander(style_table2(rb_st_tab_ex), missing = "-")
...
for (i in unique(areas)) {
  ...
  # Table caption
  cap_i <- paste0("**Table X:** Age error matrix (AEM) for area ",i,". 
                   ...
                   some cases. Only advanced readers are used. 
...
# OVERALL RANKING - All readers ###############################################

# Table caption
set.caption('**Table X:** Overall ranking of readers combines the ranking
             valuesof table 2.2, 2.3 and 2.4 and a total rank is assigned based
             on the individual ranking results of the three tables. ')

# Output table
emphasize.strong.rows((nrow(rank_tab)))
pander(style_table0(rank_tab), missing = "-")
...
# OVERALL RANKING - Advanced readers ##########################################

# Set caption
set.caption('**Table X:** Overall ranking of advanced readers.')

# Output table
emphasize.strong.rows((nrow(rank_tab_ex)))
pander(style_table0(rank_tab_ex), missing = "-")
