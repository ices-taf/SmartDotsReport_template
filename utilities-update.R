...
make_data1 <- function(dat_in, ex_level, ma_method){
  ...
  if (ex_level == "Advanced") {
    dat_un <- dat_in[dat_in$expertise == "Advanced", ]
  } else {
    dat_un <- dat_in
  }
...
make_data2 <- function(dat_in){
  ...
  var_in <- c("sample", "length", "sex", "catch_date", "ices_area", "stock",
              "year", "qtr","month","prep_method","reader", "age", "cv", "modal_age")

  dat_out <- spread(as.data.frame(dat_in)[, var_in], key = reader, value = age)
  return(dat_out)
}
...
get_cv <- function(mean_dat, std_dat, n_read, dat_in){
  ...
  # Add weighted mean
  cv2 <- rbind(cv0, get_wm(cv, dat_in, n_read, "y")[[2]])
